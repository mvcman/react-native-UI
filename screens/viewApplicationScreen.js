        ...
        onPress={() => navigation.navigate('Applicant', {data: item})}>
            <View style={[styles.listItem, styles.shadow]}>
                ...
                <View style={styles.content}>
                    <Text style={{ fontWeight: 'bold', fontSize: 18, color: theme.primary }}>{item.item.User.firstName} {item.item.User.lastName}</Text>
                    <Text style={{ fontWeight: 'normal', fontSize: 14, color: theme.textDark }}>{item.item.dateOfApplication}</Text>
                </View>
            </View>
        </TouchableOpacity>
